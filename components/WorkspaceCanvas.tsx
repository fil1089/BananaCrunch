...
const WorkspaceCanvas = forwardRef<WorkspaceCanvasRef, WorkspaceCanvasProps>(({
    ...
    images.forEach(imgData => {
      ...
      if (needsToLoad) {
        ...
        if (typeof imgData.source === 'string') {
            ...
            // FIX: The return type of `URL.createObjectURL` can be inferred as `unknown` in some environments.
            // Casting to `string` ensures type safety when assigning to `img.src`.
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    // Handle panning first for middle-click or spacebar
    if (e.button === 1 || (isSpacebarDown && !outpaintingState)) {
    ...
    if (e.button !== 0) return; // Allow only left-click for other actions
    (e.target as HTMLElement).setPointerCapture(e.pointerId);
    const pos = getMousePos(e);
...
const getResizeHandles = (image: WorkspaceImage): { corners: Record<'tl' | 'tr' | 'bl' | 'br', Point>; sides: Record<'t' | 'b' | 'l' | 'r', Point> } => ({
  corners: {
...
const getResizeHandleAtPosition = (pos: Point, image: WorkspaceImage, scale: number, allowSideHandles: boolean): ResizeHandle | null => {
  ...
  // FIX: The type of `p` from Object.entries is `unknown` in some TS environments.
  // Casting to `Point` allows safe access to its properties.
